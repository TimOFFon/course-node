Реализуйте программу, которая _синхронно_ читает файл и выводит количество, строк (\n) содержащихся в файле, в консоль, точно так же, как это сделала бы команда `cat file | wc -l`.
...
Документацию для `fs` модуля вы сможете найти по [ссылку](https://nodejs.org/api/fs.html).

Объекты типа `Buffer` используются Node для эффективного представления массивов данных, будь это ascii, бинарный или любой другой формат. Эти объекты могут быть конвертированы в строки путем вызова метода `toString()` над ними, например `const str = buf.toString()`.

Документацию для `Buffer` вы сможете найти по [ссылке](https://nodejs.org/api/buffer.html).
